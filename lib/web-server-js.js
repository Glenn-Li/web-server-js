...
/*            if (!that.mappingList[mappingId]) {
  ...
  //更新映射列表
    ...
    browser: function (list) {
		...
		console.info("Glenn -- grammar.scopeName:" + grammar.scopeName);
        if (grammar.scopeName !== 'source.gfm' && grammar.scopeName !== 'text.plain' && grammar.scopeName !== 'text.md') {
            return;
        }

        if (editor.getPath().substr(-3) !== '.md' && editor.getPath().substr(-4) !== '.txt') {
            alert('请先打开一篇文档！');
            return;
        }
        //更新地址映射
        ...
        if (editorPath.indexOf('$navigation.md') >= 0) {
            ...
            if (typeof filePath == 'undefined') {
				...
				else if (filePath.substr(-4) == '.txt') {
					filePath = filePath.replace(/\\/g, '/').replace('.txt', '');
                }*/
